...
#define STR(s) #s
#define ASSERT_RESULT(f) \
	if(VK_SUCCESS!=f){\
		throw std::runtime_error(STR(Assert result)##STR(f));\
	}
...
void Pipeline::setupDescriptorSetLayouts() {
	...
	//uniform buffer object
	{
		...
		ubo_layout.bindingCount = 1;
		ubo_layout.pBindings = &ubo_binding;

		ASSERT_RESULT(vkCreateDescriptorSetLayout(rhi->m_device, &ubo_layout, nullptr, &descriptorset_layouts[0]));
	}

	{
		...
		ASSERT_RESULT(vkCreateDescriptorSetLayout(rhi->m_device, &texture_layout, nullptr, &descriptorset_layouts[1]));
	}
}

void Pipeline::setupDescriptorSets() {
	descriptorsets.resize(rhi->m_swapchain_size);
	std::vector<VkDescriptorSetLayout> layouts(rhi->m_swapchain_size, descriptorset_layouts[0]);
	VkDescriptorSetAllocateInfo ubo_alloc = mgvk::aboutVkDescriptorSetAllocateInfo();
	ubo_alloc.descriptorPool = rhi->m_descriptor_pool;
	ubo_alloc.descriptorSetCount = rhi->m_swapchain_size;
	ubo_alloc.pSetLayouts = layouts.data();

	ASSERT_RESULT(vkAllocateDescriptorSets(rhi->m_device, &ubo_alloc, &descriptorsets[0]));

	std::vector<VkDescriptorBufferInfo> bufferinfo;
	bufferinfo.resize(rhi->m_swapchain_size);
	for (int i{ 0 }; i < rhi->m_swapchain_size; ++i) {
		bufferinfo[i] = mgvk::aboutVkDescriptorBufferInfo();
		bufferinfo[i].buffer = buffers[i];
		bufferinfo[i].offset = 0;
		bufferinfo[i].range = sizeof(UBO);
	}
	std::vector<VkWriteDescriptorSet> writeinfo;
	writeinfo.resize(rhi->m_swapchain_size);
	for (int i{ 0 }; i < rhi->m_swapchain_size; ++i) {
		writeinfo[i] = mgvk::aboutVkWriteDescriptorSet();
		writeinfo[i].descriptorCount = 1;
		writeinfo[i].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
		writeinfo[i].dstBinding = 0;
		writeinfo[i].dstSet = descriptorsets[i];
		writeinfo[i].pBufferInfo = &bufferinfo[i];
	}
	vkUpdateDescriptorSets(rhi->m_device, rhi->m_swapchain_size, writeinfo.data(), 0, nullptr);
}

void Pipeline::setupRenderpass() {
	VkAttachmentDescription colorAttachment{};
	colorAttachment.format = rhi->m_swapchain_surface_format.format;
	...
	if (vkCreateRenderPass(rhi->m_device, &renderPassInfo, nullptr, &render_pass) != VK_SUCCESS) {
		throw std::runtime_error("failed to create  render pass");
	}
}

void Pipeline::setupFrameBuffers() {
	framebuffers.resize(rhi->m_swapchain_size);
	for (int i{ 0 }; i < framebuffers.size(); ++i) {
		VkImageView attachments[2] = { rhi->m_swapchain_image_views[i],rhi->m_depth_image_view };
		VkFramebufferCreateInfo framebuffer_info = mgvk::aboutVkFramebufferCreateInfo();
		framebuffer_info.renderPass = render_pass;
		framebuffer_info.attachmentCount = 2;
		framebuffer_info.pAttachments = attachments;
		framebuffer_info.width = rhi->m_swapchain_extent.width;
		framebuffer_info.height = rhi->m_swapchain_extent.height;
		framebuffer_info.layers = 1;

		ASSERT_RESULT(vkCreateFramebuffer(rhi->m_device, &framebuffer_info, nullptr, &framebuffers[i]));
	}
}

void Pipeline::setupPieplines() {
	VkShaderModule vertShaderModule = Mage::VulkanHelper::shaderModuleCreationHelper(rhi->m_device,"E:\\Mage\\engine\\shaders\\test_vert.spv");
	VkShaderModule fragShaderModule = Mage::VulkanHelper::shaderModuleCreationHelper(rhi->m_device, "E:\\Mage\\engine\\shaders\\test_frag.spv");
	...
	viewport.x = 0.0f;
	viewport.y = 0.0f;
	viewport.width = rhi->m_swapchain_extent.width;
	viewport.height = rhi->m_swapchain_extent.height;
	...
	VkRect2D scissor{};
	scissor.offset = { 0,0 };
	scissor.extent = rhi->m_swapchain_extent;
	...
	if (vkCreatePipelineLayout(rhi->m_device, &pipelineLayoutInfo, nullptr, &pipeline_layout) != VK_SUCCESS) {
	...
	if (vkCreateGraphicsPipelines(rhi->m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS) {
		throw std::runtime_error("failed to create graphics pipeline!");
	}
	//局部变量的销毁
	vkDestroyShaderModule(rhi->m_device, fragShaderModule, nullptr);
	vkDestroyShaderModule(rhi->m_device, vertShaderModule, nullptr);
}

void Pipeline::draw(RenderModel& model) {
	void* begin;
	vkMapMemory(rhi->m_device, memories[rhi->m_current_frame_index], 0, sizeof(UBO), 0, &begin);
	...
	renderpassInfo.framebuffer = framebuffers[rhi->m_swapchain_image_index];
	renderpassInfo.renderPass = render_pass;
	renderpassInfo.renderArea.offset = { 0,0 };
	renderpassInfo.renderArea.extent = rhi->m_swapchain_extent;


	vkCmdBeginRenderPass(rhi->m_command_buffer, &renderpassInfo, VK_SUBPASS_CONTENTS_INLINE);

	vkCmdBindPipeline(rhi->m_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);

	uint32_t data{0};
	for (auto& mesh : model.meshes) {
		//update uniform buffer
		UBO ubo{};
		ubo.model = mesh.mesh_matrix;
		ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		ubo.perspective = glm::perspective(glm::radians(4.5f), rhi->m_swapchain_extent.width / (float)rhi->m_swapchain_extent.height, 0.5f, 10.0f);
		ubo.perspective[1][1] *= -1;

		memcpy(((uint8_t*)begin + data), &ubo, sizeof(UBO));

		vkCmdBindDescriptorSets(rhi->m_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 1, 1, &model.materials[mesh.material_index].set, 0, nullptr);
		...
		vkCmdBindVertexBuffers(rhi->m_command_buffer, 0, 4, buffers, offsets);
		vkCmdBindIndexBuffer(rhi->m_command_buffer, model.super_block, mesh.index_offset, VK_INDEX_TYPE_UINT32);

		uint32_t temp{ data };
		vkCmdBindDescriptorSets(rhi->m_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 0, 1, &descriptorsets[rhi->m_current_frame_index], 1, &temp);

		vkCmdDrawIndexed(rhi->m_command_buffer, mesh.index_count, 1, 0, 0, 0);
		data = data + sizeof(UBO);
		data = Mage::Mathf::RoundUp(data, 192);
	}

	vkCmdEndRenderPass(rhi->m_command_buffer);

	rhi->prepareVulkanRHIAfterRender();

	vkUnmapMemory(rhi->m_device, memories[rhi->m_current_frame_index]);
}
#pragma endregion
...
RenderModel loadModel(const std::string& file) {
	...
	ASSERT_RESULT(vkCreateDescriptorSetLayout(rhi->m_device, &layout_info, nullptr, &layout));	
	...
	for (uint32_t i{ 0 }; i < model.m_materials.size(); ++i) {
		...
		if (index_imageview_map.find(model.m_materials[i].m_pbr_metallic_roughness.m_base_color_texture.m_index) == index_imageview_map.end()) {
			...
			void* p;
			vkMapMemory(rhi->m_device, stagingMemory, 0, size, 0, &p);
			memcpy(p, model.m_textures[index].m_data.data(), size);
			vkUnmapMemory(rhi->m_device, stagingMemory);
			...
			ASSERT_RESULT(vkCreateImageView(rhi->m_device, &al_v, nullptr, &albedo.view));
			...
			ASSERT_RESULT(vkCreateSampler(rhi->m_device,&al_s,nullptr,&albedo.sampler))

			ret_model.images.push_back(albedo);
			index_imageview_map.insert(std::make_pair(index, std::make_pair(albedo.view, albedo.sampler)));

			vkDestroyBuffer(rhi->m_device, stagingBuffer, nullptr);
			vkFreeMemory(rhi->m_device, stagingMemory, nullptr);
		}
		...
		if (index_imageview_map.find(model.m_materials[i].m_pbr_metallic_roughness.m_metallic_roughness_texture.m_index) == index_imageview_map.end()) {
			...
			al_v.subresourceRange.layerCount = 1;
			al_v.subresourceRange.levelCount = 1;
			ASSERT_RESULT(vkCreateImageView(rhi->m_device, &al_v, nullptr, &m_r.view));
			...
			al_s.minLod = 0.f;
			al_s.maxLod = VK_LOD_CLAMP_NONE;
			ASSERT_RESULT(vkCreateSampler(rhi->m_device, &al_s, nullptr, &m_r.sampler))
		...
		if (index_imageview_map.find(model.m_materials[i].m_normal_texture.m_index) == index_imageview_map.end()) {
			...
			al_v.subresourceRange.layerCount = 1;
			al_v.subresourceRange.levelCount = 1;
			ASSERT_RESULT(vkCreateImageView(rhi->m_device, &al_v, nullptr, &normal.view));
			...
			al_s.minLod = 0.f;
			al_s.maxLod = VK_LOD_CLAMP_NONE;
			ASSERT_RESULT(vkCreateSampler(rhi->m_device, &al_s, nullptr, &normal.sampler))
		...
		set_alloc.descriptorPool = rhi->m_descriptor_pool;
		set_alloc.descriptorSetCount = 1;
		set_alloc.pSetLayouts = &layout;
		ASSERT_RESULT(vkAllocateDescriptorSets(rhi->m_device, &set_alloc, &material_i.set));
		...
		vkUpdateDescriptorSets(rhi->m_device, 3, write, 0, nullptr);
...
int main() {
	...
	buffers.resize(rhi->m_swapchain_size);
	memories.resize(rhi->m_swapchain_size);
	for (int i{ 0 }; i < rhi->m_swapchain_size; ++i) {
		Mage::VulkanHelper::bufferCreationHelper(
			rhi.get(), sizeof(UBO) * 100,
