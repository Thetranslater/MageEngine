...
layout(set = 1, binding = 0) readonly uniform GlobalBufferPerDrawcallFragmentShaderData
{
    PerMeshFragmentShaderData frag_datas[60];
};

layout(set = 2, binding = 0) uniform sampler2D albedo_texture;
layout(set = 2, binding = 1) uniform sampler2D metallic_texture;
...
layout(location = 2) in highp vec3 in_world_tangent;
...
void main(){
    out_final_color = texture(albedo_texture,in_texcoord) * frag_datas[in_instanceID].base_color_factor;
}