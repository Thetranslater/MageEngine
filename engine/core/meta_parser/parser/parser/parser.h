...
private:
    std::string m_project_input_file;//1

    std::vector<std::string> m_work_paths; //3
    std::string              m_module_name;//5
    std::string              m_sys_include;//4
    std::string              m_source_include_file_name; //2

    CXIndex           m_index;
    CXTranslationUnit m_translation_unit;

    std::unordered_map<std::string, std::string>  m_type_table;
    std::unordered_map<std::string, SchemaMoudle> m_schema_modules; //key is the headers which have the classes need to be reflected
