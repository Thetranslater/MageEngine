    ...
    m_sys_include(sys_include), m_module_name(module_name), m_is_show_errors(is_show_errors)
{
    ...
    /*for (auto& str : m_work_paths)
    {
        std::cout << "Constucting MetaParser: field m_work_paths : " << str << std::endl;
    }*/
...
bool MetaParser::parseProject()
{
    ...
    /*std::cout << "————————include_files————————\n";
    for (auto& str : inlcude_files)
    {
...
{
    bool parse_include_ = parseProject(); //这一步生成了parser_header.h该文件,文件路径由参数给出
    if (!parse_include_)
    {
...
void MetaParser::buildClassAST(const Cursor& cursor, Namespace& current_namespace)
{
    for (auto& child : cursor.getChildren())
    {
        ...
        {
            //假如cursor指出了一个拥有定义的class或struct，那么我们可以根据cursor.getSourceFile找到该类定义的文件，将该文件注册到m_schema_modules中
        ...
        {
            //假如该cursor不是定义且指向了一个命名空间，那么对该命名空间下递归遍历cursor
